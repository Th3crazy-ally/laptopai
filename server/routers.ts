...
const requirementsSchema = z.object({
  ...
  brandPreference: z.string().max(80).optional(),
  osPreference: z.string().max(40).optional(),
  gpuRequired: z.boolean().default(false),
  minVram: z.number().min(1).max(32).optional(),
  maxWeight: z.number().min(0.5).max(5).optional(),
  displayPreference: z.enum(["any", "high-refresh", "color-accurate"]).default("any"),
  gamingImportance: z.number().min(0).max(1).default(0.2),
  programmingImportance: z.number().min(0).max(1).default(0.2),
  editingImportance: z.number().min(0).max(1).default(0.2),
  aiImportance: z.number().min(0).max(1).default(0.2),
