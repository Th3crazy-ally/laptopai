...
const initialRequirements: RecommendationRequirements = {
  ...
  gamingImportance: 0.45,
  programmingImportance: 0.8,
  editingImportance: 0.45,
  aiImportance: 0.35,
            ...
            {mode === "brief" ? (
              ...
              <div id="guided-form" className="guided-panel">
                ...
                <div className="form-row form-row-budget">
                  <div className="form-field"><Label htmlFor="cpu-preference">CPU preference</Label><Input id="cpu-preference" value={requirements.cpuPreference ?? ""} placeholder="e.g. Ryzen 7, Core Ultra" onChange={(event) => updateRequirement("cpuPreference", event.target.value || undefined)} /></div>
                  <div className="form-field"><Label htmlFor="gpu-preference">GPU preference</Label><Input id="gpu-preference" value={requirements.gpuPreference ?? ""} placeholder="e.g. RTX 4060" onChange={(event) => updateRequirement("gpuPreference", event.target.value || undefined)} /></div><div className="form-field"><Label htmlFor="brand-preference">Brand preference</Label><Input id="brand-preference" value={requirements.brandPreference ?? ""} placeholder="Any brand" onChange={(event) => updateRequirement("brandPreference", event.target.value || undefined)} /></div>
                  <div className="form-field"><Label htmlFor="display-preference">Display priority</Label><select id="display-preference" value={requirements.displayPreference ?? "any"} onChange={(event) => updateRequirement("displayPreference", event.target.value as RecommendationRequirements["displayPreference"])}><option value="any">Any good display</option><option value="high-refresh">High refresh rate</option><option value="color-accurate">Color accurate / OLED</option></select></div>
                  <div className="form-field"><Label htmlFor="gpu-required">Dedicated GPU</Label><select id="gpu-required" value={requirements.gpuRequired ? "required" : "optional"} onChange={(event) => updateRequirement("gpuRequired", event.target.value === "required")}><option value="optional">Optional</option><option value="required">Required</option></select></div>
                </div>
                <Separator />
